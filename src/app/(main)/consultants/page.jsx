...
import ConsultantSearch from "@/components/ui/ConsultantSearch";
...
export default async function AllConsultantsPage({ searchParams }) {
  ...
  const filteredConsultants = consultants.filter((doctor) => {
    ...
    <div className="w-full bg-[#F9F9F9] dark:bg-zinc-950 min-h-screen pb-24 space-y-12">
      ...
      <main className="max-w-7xl mx-auto px-6 sm:px-8 lg:px-12">
        ...
        {filteredConsultants.length === 0 ? (
          <div className="text-center py-16 border border-dashed border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 p-8">
            <h3 className="text-lg font-bold text-zinc-700 dark:text-zinc-300 uppercase tracking-wider">
              No Consultants Found
            </h3>
            <p className="text-sm text-zinc-400 mt-1">
              We couldn&apos;t find matches for &quote;{resolvedParams?.search}
              &quote;. Try checking your spelling.
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 lg:gap-10">
            {filteredConsultants.map((doctor) => {
              const spec =
